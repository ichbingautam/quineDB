...
RespParser::Result RespParser::consume(const uint8_t *data, size_t len,
                                       size_t &consumed) {
  size_t pos = 0;
  ...
  while (pos < len) {
    ...
    case State::WaitSize: {
      ...
      // \r found)
      std::string size_str(reinterpret_cast<const char *>(data + start),
                           pos - start);
      try {
    ...
    case State::WaitArgSize: {
      if (data[pos] != '$')
        return Result::Error; // Expected bulk string marker
      pos++;
      ...
      std::string len_str(reinterpret_cast<const char *>(data + start),
                          pos - start);
      try {
